...
def session_viewer(args):
    ...
    for user_action_index, user_action in enumerate(session["userActions"][2:]):
        ...
        for keyframe_index in range(
            ...
            # 3D pointcloud
            render_camera_obj = sim._sensors['color_sensor']._sensor_object.render_camera
            # voxel_repr = np.zeros((render_camera_obj.viewport[0], render_camera_obj.viewport[1]))
            coordinates_camera_frame = {'X': [], 'Y': [], 'Z': [], 'C': []}
            for i in range(1, render_camera_obj.viewport[0], 5):
                for j in range(1, render_camera_obj.viewport[1], 5):
                    import ipdb; ipdb.set_trace()

                    origin = sim._sensors['color_sensor']._sensor_object.render_camera.unproject(mn.Vector2i(j, i)).origin
                    dir_vec = sim._sensors['color_sensor']._sensor_object.render_camera.unproject(mn.Vector2i(j, i)).direction
                    coordinates_camera_frame['X'].append(dir_vec[0] * obs['depth_sensor'][j][i]/dir_vec[2] + origin[0])
                    coordinates_camera_frame['Y'].append(dir_vec[1] * obs['depth_sensor'][j][i]/dir_vec[2] + origin[1])
                    coordinates_camera_frame['Z'].append(obs['depth_sensor'][j][i] + origin[2])
                    coordinates_camera_frame['C'].append(obs['color_sensor'][j][i][:3]/255)
            
            # save 
            np.savez('coordinate_camera_frame', 
                X=coordinates_camera_frame['X'],
                Y=coordinates_camera_frame['Y'],
                Z=coordinates_camera_frame['Z'],
                C=coordinates_camera_frame['C']
            )
            # plot
            fig = plt.figure()
            ax = plt.axes(projection='3d')
            ax.scatter3D(
                coordinates_camera_frame['X'],
                coordinates_camera_frame['Y'],
                coordinates_camera_frame['Z'],
                c=coordinates_camera_frame['C']
            )
